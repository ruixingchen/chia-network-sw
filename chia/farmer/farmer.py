...
class Farmer:
    ...
    async def get_reward_targets(self, search_for_private_key: bool, max_ph_to_search: int = 500) -> Dict:
        pool_target_encoded = self.og_pooling.get_pool_target_encoded()
        if search_for_private_key:
            all_sks = await self.get_all_private_keys()
            have_farmer_sk, have_pool_sk = False, False
            pool_target: bytes32 = self.og_pooling.get_pool_target()
            search_addresses: List[bytes32] = [self.farmer_target, pool_target]
            for sk, _ in all_sks:
                ...
                if not have_pool_sk and pool_target in found_addresses:
                    search_addresses.remove(pool_target)
                    have_pool_sk = True
            ...
            return {
                ...
                "pool_target": pool_target_encoded,
